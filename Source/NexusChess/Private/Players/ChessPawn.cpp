...
void AChessPawn::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	//-----------------Axis Maping-----------------
	PlayerInputComponent->BindAxis("MoveForward/Backwards", this, &AChessPawn::IE_MoveForwardBackward);
	PlayerInputComponent->BindAxis("MoveRight/Left", this, &AChessPawn::IE_MoveRightLeft);
	PlayerInputComponent->BindAxis("LookLeft/Right", this, &AChessPawn::IE_TurnLeftRight);
	PlayerInputComponent->BindAxis("LookUp/Down", this, &AChessPawn::IE_LookUpDown);
...
void AChessPawn::PostInitializeComponents()
{
	Super::PostInitializeComponents();
	//GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Green, FString::Printf(TEXT("POST"), this->_allMaterials.Num()));	
}
...
void AChessPawn::IE_MoveForwardBackward(float value)
{
	if (!APawn::Controller)
		return;
	...
	else if (value == 0.0f && this->_currentSpeedValue != 0.0f)
	{
		if (this->_currentSpeedValue > 0.2f)
			...
			this->_currentSpeedValue = 0;
	}
	else if (this->_currentSpeedValue == 0.0f)
		return;
...
void AChessPawn::IE_MoveRightLeft(float value)
{
	if (!APawn::Controller)
		return;
	...
	else if (this->_currentDirectionValue == 0.0f)
		return;
	const FRotator yawRotation(0.0f, APawn::Controller->GetControlRotation().Yaw, 0.0f);
	const FVector direction(FRotationMatrix(yawRotation).GetUnitAxis(EAxis::Y));
	AddMovementInput(direction, this->_currentDirectionValue);
}
void AChessPawn::IE_TurnLeftRight(float value)
{
...
void AChessPawn::IE_LookUpDown(float value)
{
	if (APawn::Controller && value != 0.0f)
	{
...
void AChessPawn::IE_TragetArmLength(float value)
{
	if (!APawn::Controller || (this->_currentTargetArmValue == 0.0f && value == 0.0f))
		return;
	...
	if (targetArm > 3000.0f)
		targetArm = 3000.0f;
	else if (targetArm < 200.0f)
		targetArm = 200.0f;
	this->_cameraBoom->TargetArmLength = targetArm;
	//if (GEngine)
	//	GEngine->AddOnScreenDebugMessage(-1, 1.0f, FColor::Red, FString::Printf(TEXT("1111111111111 : % f"), targetArm));
}
void AChessPawn::IE_CameraFOV(float value)
{
	if (!APawn::Controller || (this->_currentCameraFOV == 0.0f && value == 0.0f))
		return;
